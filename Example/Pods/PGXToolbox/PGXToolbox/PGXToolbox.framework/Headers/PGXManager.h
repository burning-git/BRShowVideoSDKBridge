...
typedef void(^PGXStartCompletionBlock)(BOOL initStatus, NSDictionary *userInfo);
typedef void(^PGXLoginCompletionBlock)(BOOL loginStatus, NSDictionary *userInfo);

@interface PGXManager : NSObject
...
/// @param config 初始化配置
+ (void)initializeWithConfigPath:(NSString *)configPath config:(id)config;

/// 主线程注册,不注册会导致SDK功能无法正常使用。
/// @param completionBlock 初始化回调
+ (void)startWithCompleteHandler:(PGXStartCompletionBlock)completionBlock;
 ...
 */
+ (void)loginWithParamsString:(NSString *)paramsString completionBlock:(PGXLoginCompletionBlock)completionBlock;
/**
 * 退出登录
 */
+ (void)logoutWithCompletionBlock:(PGXLoginCompletionBlock)completionBlock;
