//
//  RangersAPMForSDK+CrashCallback.h
//
...
typedef void (*apm_crash_callback)(char * _Nonnull * _Nonnull dynamic_key, char * _Nonnull * _Nonnull dynamic_data, uint64_t crash_time, uint64_t fault_address, thread_t current_thread, thread_t crash_thread);

/**
 RangersAPM 内部处理完 Crash 后，回调业务方，自定义数据，会在 APMPlus 平台 自定义数据中展示
...
@interface RangersAPMForSDK (CrashCallback)
