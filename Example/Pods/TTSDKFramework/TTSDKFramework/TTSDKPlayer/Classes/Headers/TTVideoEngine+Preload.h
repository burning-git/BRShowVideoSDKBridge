...
@property(nonatomic, assign) NSInteger netSpeedUpdateInterval; // Android: NET_SPEED_UPDATE_INTERVAL;
...
/// Play media with url source, use mdl to load data.
/// @param urlSource url souce model
- (void)ls_setDirectURLSource:(TTVideoEngineUrlSource *)urlSource;
...
/// set MDL groupID, use to correlate play event and preload event
...
/**
 ...
 It may not be a clear operation, because some data for cache marked useful.
 */
...
/**
 Clear all cache data.
 If force is YES, it will force clear cache rightnow.
 */
+ (void)ls_clearAllCaches:(BOOL)force;

/**
 get the clear size by ls_clearAllCaches.
 return byte
 */
...
/**
 quick query cache,can replace ls_tryQuickGetCacheSizeByKey
 do not fallbackt to read file
 */
...
/// precise preload Timestamp(ms); SDK solution;
@property (nonatomic, assign) NSInteger preloadTimestamp;
/// precise preload duration(ms); SDK solution;
...
// bugfix: ts url key use absolute url
